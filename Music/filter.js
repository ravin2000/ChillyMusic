...
module.exports = {
  name: "filter",
  description: "Hanghatások beállítása.",
  aliases: ["fi"],
  cooldown: 3,
  edesc: `Írja be ezt a parancsot az aktuális hanghatás stílusának megváltoztatásához \nHasználata: ${PREFIX}filter <Szűrőtípus>`,
...
async execute(message, args, client) {
    ...
    if (message.channel.activeCollector)
      return attentionembed(message, "A keresés aktív!");
    //if the user is not in a voice channel return error
    if (!message.member.voice.channel)
      return attentionembed(message, "Kérlek csatlakozz egy hangcsatornához.")
    ...
    switch (args[0]) {
      ...
      default:
        ...
        .setColor("YELLOW")
        .setTitle("Ez a filter nem létezik! Válassz ezekből:")
        .setDescription(`
        \`bassboost\`
        \`8D\`
        \`vaporwave\`
        ...
        \`pulsator\`
        \`subboost\`
        \`clear\`   ---  leszedi az összes filtert`)
        .setFooter(`Például: ${PREFIX}filter bassboost`)
        )
        break;
    }
    //set choice to zero
