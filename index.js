...
let statuszok = [
        "Prefix : ! ->alap commandok, m? ->zene commandok",
        "Fejlesztő: 𝓇𝒶𝓋𝒾𝓃#2215",
        "Nemsokára húsvét",
    ]
...
client.on(`message`, async (message) => {
  ...
  if (message.content.toLowerCase() === `${PREFIX}uptime`) {
   ...
   return message.channel.send(`***__SuperBot Music ennyi ideje elérhető:__***\n\`\`\`fix\n${days}d ${hours}h ${minutes}m ${seconds}s\n\`\`\``);
}
...
//command Handler DO NOT TOUCH
 ...
 if (timestamps.has(message.author.id)) {
   ...
   if (now < expirationTime) {
     ...
     return message.reply(
      ...
      .setTitle(`❌ Kérlek várj \`${timeLeft.toFixed(1)} másodpercet\` mielőtt újra felhasználná a \`${PREFIX}${command.name}\`!`)    
     );
   }
 }
 ...
 try {
   ...
   console.error(error);
   message.reply(`Hiba történt a parancs végrehajtásakor.`).catch(console.error);
 }
